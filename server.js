...
async function generateRecap(chatLogs) {
  ...
  const customPrompt = `
You are a Twitch stream assistant.
...
`.trim();
  ...
  console.log(
    `[Gemini] Sending ${chatLogs.length} messages for recap...`
  );
...
testGeminiBtn
  .addEventListener(
    ...
    async () => {
      ...
      testGeminiBtn.disabled =
        true;

      testResult.innerHTML =
        '<span style="color:#adadb8;">Testing Gemini API...</span>';
      ...
      try {
        ...
        if (
          ...
          testResult.innerHTML =
            '<strong style="color:#00f59b;">API Success!</strong>' +
            '<pre>' +
            escapeHtml(
              data.output
            ) +
          ...
          testResult.innerHTML =
            '<strong style="color:#ff4f4f;">API Error Details:</strong>' +
            '<pre>' +
            escapeHtml(
              JSON.stringify(
                data.error,
        ...
        testResult.innerHTML =
          '<span style="color:#ff4f4f;">Failed to connect to backend endpoint.</span>';
...
client.on(
  ...
  async (
    ...
    if (
      ...
      // Need at least 5 organic messages
      if (
        ...
        try {
          ...
          console.error(
            'Failed to send history warning:',
            err
          );
      ...
      // Prevent simultaneous recap calls
      lastRecapUse =
        now;
      ...
      try {
        ...
        // Gemini failed:
        // remove cooldown so we can retry
        lastRecapUse =
          0;
