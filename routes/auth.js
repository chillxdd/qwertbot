...
function registerAuthRoutes(app, options) {
  ...
  app.get('/auth/twitch/callback', async (req, res) => {
    ...
    try {
      ...
      return res.send(`<!doctype html><html><body style="font-family:Arial;background:#0f0f12;color:white;padding:40px"><div style="max-width:700px;margin:auto;background:#18181b;padding:24px;border-radius:8px"><h2 style="color:#00f59b">Bot authorization successful</h2><p>Authorized account: <strong>${escapeHtml(authorizedLogin)}</strong></p><p>The bot grant now includes the scopes used for Twitch's modern Chat API, the legacy IRC connection used to receive chat, temporary hourly-recap pinning, random current-chatter selection for custom commands, and native Twitch clip creation.</p><p>Return to the dashboard and complete broadcaster authorization if it is still pending.</p><p><a style="color:#bf94ff" href="${escapeHtml(adminPath)}">Return to dashboard</a></p></div></body></html>`);
    } catch (err) {
