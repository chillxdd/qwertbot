...
function registerMemoryRoutes(app, { requireModSession, getDatabaseConnected, getBotPersonalityManager, getRecapManager, channelName }) {
  ...
  app.post('/viewer-profiles/facts-bulk-delete', requireModSession, async (req, res) => {
    if (!getDatabaseConnected()) return res.status(503).json({ success: false, error: 'MongoDB is not connected.' });
    try {
      const result = await deleteViewerFactsByConfidence(channelName, {
        maxConfidence: req.body?.maxConfidence,
        approvalScope: req.body?.approvalScope
      });
      return res.json({ success: true, ...result });
    } catch (err) {
      return res.status(400).json({ success: false, error: err.message || 'Could not bulk-delete viewer observations.' });
    }
  });

  app.post('/viewer-profiles/clear-all', requireModSession, async (req, res) => {
    if (!getDatabaseConnected()) return res.status(503).json({ success: false, error: 'MongoDB is not connected.' });
    try {
      const result = await clearAllViewerProfiles(channelName);
      return res.json({ success: true, ...result });
    } catch (err) {
      return res.status(400).json({ success: false, error: err.message || 'Could not clear viewer profiles.' });
    }
  });
