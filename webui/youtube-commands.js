...
function renderCustomCommands() {
  ...
  if (!commandsEngineEnabled) {
    ...
    list.innerHTML = visible.map((command) => {
      const triggers = Array.isArray(command.triggers) ? command.triggers : [];
      const chips = triggers.map((trigger) => `<span class="custom-trigger-chip"><strong>${esc(trigger.trigger)}</strong><small>!Command</small></span>`).join('');
...
async function loadCommands() {
  ...
  try {
    ...
    $('readonlyNativeCommandList').innerHTML = '';
    $('readonlyCustomCommandPagination').hidden = true;
    $('readonlyNativeCommandPagination').hidden = true;
    msg.textContent = err?.message || 'Could not load public commands.';
    $('readonlyNativeCommandsMsg').textContent = '';
  }
}
